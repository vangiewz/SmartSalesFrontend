...
export default function PromptBar({ loading, value, onChange, onRun, onDownload }: Props) {
  return (
    <div className="bg-white rounded-2xl shadow p-4 sm:p-6 border">
      ...
      <div className="mt-3 flex flex-col sm:flex-row gap-2">
        ...
        <div className="flex gap-2">
          <button onClick={() => onDownload("xlsx")} disabled={loading||!value.trim()} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl border hover:bg-gray-50">
            <Download className="w-4 h-4" /> Excel
          </button>
          <button onClick={() => onDownload("pdf")} disabled={loading||!value.trim()} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl border hover:bg-gray-50">
