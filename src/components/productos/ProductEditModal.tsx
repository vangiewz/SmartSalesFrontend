// src/components/productos/ProductEditModal.tsx
import { useState } from 'react'
import { X } from 'lucide-react'
import type { Marca, TipoProducto, Producto, ProductoFormData } from '../../types/producto'
...
interface ProductEditModalProps {
  ...
  tipos: TipoProducto[]
  onClose: () => void
  onSave: (id: number, data: Partial<ProductoFormData>) => Promise<void>
}
...
export default function ProductEditModal({
  ...
  const [tiempoGarantia, setTiempoGarantia] = useState(String(producto.tiempogarantia))
  const [marcaId, setMarcaId] = useState(String(producto.marca_id))
  const [tipoId, setTipoId] = useState(String(producto.tipoproducto_id))
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl max-w-2xl w-full my-8">
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-purple-100">
            <h2 className="text-xl sm:text-2xl font-bold text-purple-900">✏️ Editar Producto</h2>
            <button
              type="button"
              onClick={onClose}
